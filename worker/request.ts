import { validateNameAnalysis, type NameAnalysis } from "../src/name-match.js";
...
export async function parseFindRequest(request: Request) {
  ...
  const allowedKeys = new Set(["name", "context", "continuation", "contextInterpretation", "nameAnalysis"]);
  ...
  let nameAnalysis: NameAnalysis | undefined;
  if (body.nameAnalysis !== undefined) {
    try {
      nameAnalysis = validateNameAnalysis(body.nameAnalysis, name);
    } catch {
      throw new RequestError("Name analysis is invalid.", 400);
    }
  }
  return {
    name,
...
export async function parseNameRequest(request: Request) {
  const contentType = request.headers.get("content-type")?.split(";", 1)[0].trim().toLowerCase();
  if (contentType !== "application/json") {
    throw new RequestError("Content-Type must be application/json.", 415);
  }
  ...
  if (!isPlainObject(body) || Object.keys(body).some((key) => key !== "name")) {
    throw new RequestError("Request contains unsupported fields.", 400);
  }
  const name = typeof body.name === "string" ? body.name.trim() : "";
  if (!name || name.length > MAX_NAME_LENGTH || /[\u0000-\u001f\u007f]/u.test(name)) {
